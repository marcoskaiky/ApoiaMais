    ...
    public function index(Request $request)
    {
        ...
        foreach ($todosItens as $item) {
            ...
            if ($doacoesDoItem->isEmpty()) {
                ...
                foreach ($doacoesDoItem as $chave => $itemAgrupado) {
                    // Calcular quantidade real em estoque (recebidos - enviados)
                    $quantidadeRecebida = $itemAgrupado['quantidade_total'];
                    $quantidadeEnviada = $itensAgrupadosEnviados->get($chave, 0);
                    $quantidade = $quantidadeRecebida - $quantidadeEnviada;

                    // Apenas adicionar se houver quantidade positiva
                    if ($quantidade > 0) {
