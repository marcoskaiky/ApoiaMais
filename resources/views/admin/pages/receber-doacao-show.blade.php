...
<div class="content-wrapper">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Detalhes da Doação</h1>
        <a href="{{ route('admin.receber-doacaos.index', ['tab' => 'historico']) }}" class="btn btn-secondary">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Doação #{{ $receberDoacao->id }}</h5>
            <button type="button" class="btn btn-light btn-sm" id="btnEditar">
                <i class="fas fa-edit"></i> Editar
            </button>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.receber-doacaos.update', $receberDoacao->id) }}" method="POST" id="formDoacao">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        <label class="form-label fw-bold">Doador</label>
                        <input type="text" class="form-control" value="{{ $receberDoacao->doador->nome }}" disabled>
                        <input type="hidden" name="doador_id" value="{{ $receberDoacao->doador_id }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-bold">CPF/CNPJ</label>
                        <input type="text" class="form-control" value="{{ $receberDoacao->doador->cpf ?? $receberDoacao->doador->cnpj }}" disabled>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" value="{{ $receberDoacao->campanha->nome ?? 'Nenhuma' }}" disabled>
                        <input type="hidden" name="campanha_id" value="{{ $receberDoacao->campanha_id }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-bold">Data da Doação</label>
                        <input type="text" class="form-control" value="{{ $receberDoacao->created_at->format('d/m/Y H:i') }}" disabled>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-12">
                        ...
                        <textarea class="form-control campo-editavel" name="observacoes" rows="3" disabled>{{ $receberDoacao->observacoes }}</textarea>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach($receberDoacao->itens as $index => $itemDoacao)
                        ...
                        <tfoot>
                            <tr class="table-light">
                                ...
                                <td class="fw-bold">{{ $receberDoacao->itens->sum('quantidade') }}</td>
                                <td colspan="3"></td>
